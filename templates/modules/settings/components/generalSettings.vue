<template>
  <div class="bg-dark_white !p-5 rounded-[20px] flex flex-col gap-6">
    <div class="flex items-center justify-between">
      <h3 class="text-dark_900 !text-[16px] !font-[600]">{{ $t('site_info') }}</h3>
    ...
    <GenericForm id="myForm" :schema="settingsStore.generalSettings" @submit="updateSettings" />
  </div>
</template>
...
const updateSettings = (value) => {
  ...
  let formData = handleFormData(value, 'settings', 'general_settings')
  let { errors } = settingsStore.updateSettings(formData)
  if (errors) handleErrors(settingsStore.generalSettings, errors)
